...
data Suit = Club | Diamond | Heart | Spade deriving (Show, Eq, Enum)
...
rankValue rank = toInteger $ (fromEnum rank) + 2

suitValue :: Suit -> Integer
suitValue suit = 1 -- toInteger $ (fromEnum suit) + 1

valueRank :: Integer -> Rank
valueRank value = toEnum (fromInteger value - 2) :: Rank

highCard :: [String] -> Rank
highCard hand = maximum $ map charToRank $ map (!! 0) hand

count :: Eq a => [a] -> a -> Int
count ls x = length $ filter (== x) ls

generateEnumValues :: (Enum a) => [a]
generateEnumValues = enumFrom (toEnum 0)

ranks :: [Rank]
ranks = generateEnumValues

countCards :: [String] -> [(Rank, Int)]
countCards hand =
    let hand1 = sort $ map charToRank $ map (!! 0) hand
        hand2 = cartProd ranks [hand1] in
            reverse $ countCardsAux hand2 where
                countCardsAux [] = []
                countCardsAux (h : t) =
                    let (rank, hnd) = h
                        c = count hnd rank in
                            (rank, c) : countCardsAux t        

highCard2 :: [String] -> Maybe Rank
highCard2 hand =
    let counts = countCards hand
        hand1 = filter (\(_, c) -> c == 1) counts
        hand2 = map fst hand1 in
            if length hand2 == 0 then Nothing
            else Just (maximum $ hand2)

pair :: [String] -> Maybe Rank
pair hand =
    let counts = countCards hand in
        pairsAux counts where
            pairsAux [] = Nothing
            pairsAux (h : t) =
                let (rank, c) = h in
                    if c == 2 then Just rank
                    else pairsAux t

twoPair :: [String] -> [Rank]
twoPair hand =
    let counts = countCards hand in
        twoPairAux counts where
            twoPairAux [] = []
            twoPairAux (h : t) =
                let (rank, c) = h in
                    if c == 2 then rank : twoPairAux t
                    else twoPairAux t

triple :: [String] -> Maybe Rank
triple hand =
    let counts = countCards hand in
        pairsAux counts where
            pairsAux [] = Nothing
            pairsAux (h : t) =
                let (rank, c) = h in
                    if c == 3 then Just rank
                    else pairsAux t

straight :: [String] -> Bool
straight hand =
    let counts = countCards hand
        num = intListToInt $ map snd counts in
            (mod num 11111) == 0

flush :: [String] -> Maybe Suit
flush hand =
    let suits = removeDups $ map charToSuit $ map (!! 1) hand in
        if length suits == 1 then Just (suits !! 0)
        else Nothing

fullHouse :: [String] -> Bool
fullHouse hand =
    let p = isJust $ pair hand
        t = isJust $ triple hand in
            p && t

quads :: [String] -> Maybe Rank
quads hand =
    let counts = countCards hand in
        pairsAux counts where
            pairsAux [] = Nothing
            pairsAux (h : t) =
                let (rank, c) = h in
                    if c == 4 then Just rank
                    else pairsAux t

straightFlush :: [String] -> Bool
straightFlush hand =
    let s = straight hand
        f = isJust $ flush hand in
            s && f

royalFlush :: [String] -> Bool
royalFlush hand =
    let s = straightFlush hand
        h = (highCard hand) == Ace in
            s && h

scoreTwoPair :: [String] -> Integer
scoreTwoPair hand =
    let tp = twoPair hand in
        if length tp == 2 then
            let hp = rankValue (tp !! 0) * 1_000_000
                lp = rankValue (tp !! 1) * 10_000 in
                    hp + lp
        else 0

scoreHand :: [String] -> Integer
scoreHand hand =
    let hc = maybe 0 rankValue $ highCard2 hand
        pa = maybe 0 rankValue $ pair hand
        tp = scoreTwoPair hand
        tr = maybe 0 rankValue $ triple hand
        st = toInteger $ fromEnum $ straight hand 
        fl = maybe 0 suitValue $ flush hand
        fh = toInteger $ fromEnum $ fullHouse hand
        qu = maybe 0 rankValue $ quads hand
        sf = toInteger $ fromEnum $ straightFlush hand
        rf = toInteger $ fromEnum $ royalFlush hand in
            sum [hc,
                 pa *                   100,
                 tp, --              10_000
                     --           1_000_000
                 ...
                 fl *        10_000_000_000,
                 fh *       100_000_000_000,
                 qu *    10_000_000_000_000,
                 sf *   100_000_000_000_000,
                 rf * 1_000_000_000_000_000]

result :: Integer
result =
    resultAux tests where
        resultAux [] = 0
        resultAux (h : t) =
            let p1 = take 5 h
                p2 = drop 5 h
                p1s = scoreHand p1
                p2s = scoreHand p2 in
                    if p1s > p2s then 1 + resultAux t
                    else resultAux t

-- :load 0054.hs
-- rankValue $ charToRank 'Q'
-- map (!! 0) ["5H", "5C", "6S", "7S", "KD"]
-- map charToRank $ map (!! 0) ["5H", "5C", "6S", "7S", "KD"]
-- maximum $ map charToRank $ map (!! 0) ["5H", "5C", "6S", "7S", "KD"]
-- highCard ["5H", "5C", "6S", "7S", "KD"]
-- count Five [Five,Five,Six,Seven,King]
-- countCards ["5H", "5C", "6S", "7S", "KD"]
-- pair ["5H", "5C", "6S", "7S", "KD"]
-- twoPair ["5H", "5C", "6S", "6S", "KD"]
-- triple ["5H", "5C", "5S", "6S", "KD"]
-- straight ["2H", "3C", "4S", "5S", "6D"]
-- flush ["2H", "6H", "4H", "5H", "3H"]
-- fullHouse ["2H", "2D", "4C", "4D", "4S"]
-- quads ["2H", "4D", "4C", "4H", "4S"]
