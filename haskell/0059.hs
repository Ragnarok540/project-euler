...
-- three lower case characters. Using 0059_cipher.txt (right click
...
passwords = let ran = [97..122] in
    [(x, y, z) | x <- ran, y <- ran, z <- ran]

decode :: [Int] -> (Int, Int, Int) -> [Int]
decode [] _ = []
decode (a : b : c : t) (x, y, z) =
    [xor a x, xor b y, xor c z] ++ decode t (x, y, z)

contains :: [[Int]] -> [Int] -> Bool
contains [] _ = True
contains (h : t) text =
    isInfixOf h text && contains t text

crack :: [Int] -> [[Int]] -> (Int, Int, Int)
crack text test =
    let pws = passwords in
        crackAux pws text test where
            crackAux [] _ _ = (0, 0, 0)
            crackAux (h : t) tx ts =
                let plain = decode tx h in
                    if contains ts plain then h
                    else crackAux t tx ts

result :: Int
result = sum $ decode cipher (101, 120, 112)

-- :load 0059.hs
-- map toEnum $ decode cipher (101, 120, 112) :: String
-- [116,104,101] the
-- [116,104,97,116] that
-- crack cipher [[116,104,101], [116,104,97,116]]
-- 129448
