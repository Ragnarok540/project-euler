...
import NumberUtils (palindromeInt, reverseNum)

isLychrel :: Integer -> Bool
isLychrel num =
    isLychrelAux num 0 where
        isLychrelAux _ 50 = True
        isLychrelAux n counter =
            let r = reverseNum n
                s = n + r in
                    if palindromeInt s then False
                    else isLychrelAux s (counter + 1)

result :: Int
result = sum $ map fromEnum $ map isLychrel [196..10000]
