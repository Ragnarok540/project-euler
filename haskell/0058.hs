...
primeRatio ls =
    let ls1 = map toInteger ls
        ls2 = filter isPrime ls1 in
            (fromIntegral $ length ls2) / (fromIntegral $ length ls)

result :: Int -> Double
result size = primeRatio $ spiralDiagonals size $ ulamSpiral size

-- :load 0058.hs
-- ulamSpiral 7
-- spiralDiagonals 7 $ ulamSpiral 7
-- primeRatio [1,3,5,7,9,13,17,21,25,31,37,43,49]
